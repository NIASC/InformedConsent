...
if ($info_id > 0 || $siteData['slug_END']) {
	...
	if ($get_info_result = DBM::queryData($get_info_query, $database_link)) {
		...
		$siteData['section_img'] = $get_info_data->thumbnail;
	...
	if( isset($_GET['year']) ) {
		...
		$year = (int)extractGet('year');
		
		$addQuery = " AND YEAR(`start_date`) = '".$year."'";
	}
	if(!$siteData['friendly_user'] || !isset($_GET['preview'])) $addQuery = " AND `published` = 'Y' ".$addQuery;
	...
	if ($get_info_result = DBM::queryData($get_info_query, $database_link)) {
		...
		while ($get_info_data = DBM::fetchObject($get_info_result)) {
			...
			$list_data[] = array (
								...
								'description' => trim_text($get_info_data->description, $language['char_size']*250),
								'thumbnail' => $thumbnail,
	...
	$smarty->assign('content_file', 'news_page.tpl');
}
